...
export function CoverageCard({ link }: { link: CoverageLink }) {
  ...
  return (
    <div className="card card-pad flex flex-col gap-3">
      ...
      <Metric resolved={link.resolved} universe={link.universe} unitLabel="resolved" compact />

      <div>
        <div className="h-2 w-full overflow-hidden rounded-full bg-ink-100"
             role="img"
             aria-label={`${link.resolved} of ${link.universe} resolved, ${pct.toFixed(1)} percent`}>
          <div className={`h-full rounded-full ${low ? "bg-ev-e3" : "bg-pep-700"}`}
               style={{ width: `${Math.min(100, pct)}%` }} />
        </div>
        <div className="subtle num mt-1">Gap: {link.gap.toLocaleString("en-US")} applications</div>
      </div>

      {link.breakdown?.length ? (
        <div className="rounded border border-ink-200 bg-pep-50 p-2">
          <div className="label mb-1">Two sources of different authority</div>
          ...
          <p className="mt-1.5 text-[11px] leading-snug text-ink-500">
            They are not presented as equivalent: the E3 share was derived from free text and
      ...
      <dl className="mt-auto space-y-1 border-t border-ink-100 pt-2 text-[11px]">
        <div className="flex gap-1.5">
          <dt className="shrink-0 text-ink-400">Source</dt>
          <dd className="text-ink-600">{link.source}</dd>
        </div>
        <div className="flex gap-1.5">
          <dt className="shrink-0 text-ink-400">Unblocks</dt>
